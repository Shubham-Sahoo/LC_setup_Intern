...
class NNResidualFeaturizer:
    ...
    def obs_to_curtain_info(self,
                            obs: BaseEnv.Observation) -> CurtainInfo:
        lc_ranges, lc_image = obs[0].lc_ranges, obs[0].lc_image

        i = utils.hits_from_lc_image(lc_image, ithresh=self._LC_INTENSITY_THRESH)  # (C,)
        r = lc_ranges  # (C,)
        t = self._thetas
