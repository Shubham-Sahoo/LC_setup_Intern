...
from policies.velocity import VelocityActor, arrow
import utils
...
class SecondBackend:
    def __init__(self):
        self.env = BaseEnv(debug=False,
                           ...
                           preload=True,
                           progress=True)
        self.actor = VelocityActor(transform_w2c = self.env.light_curtain.lc_device.TRANSFORMS["wTc"], thetas=self.env.thetas)
        self.arrow = arrow(self.env.simpy_env)
        ...
        vizstream(app, self.arrow.streaming, astype="arrows")
    ...
    def run_simulation(self, idx):
        ...
        self.arrow.reset(self.env.simpy_env)
        ...
        while not done:
            ...
            # take step
            obs, rew, done, e_info = self.env.step(act, self.actor.latency, demonstration=True)

            self.arrow.pub(self.actor)
