...
from devices.set.models.GAT import GAT_Model
...
class Node_feat(MessagePassing):
    """
    Edge Feature Multiplication Message Passing Network Class. Extracted from torch_geometric, with minor modifications.
    (https://rusty1s.github.io/pytorch_geometric/build/html/modules/nn.html)
    """
    def __init__(self, in_channels,fc_dims, out_channels, edge_model=None, nedge_features=4):
        super(Node_feat, self).__init__(aggr='max') #  "Max" aggregation.
        fc_dims.append(out_channels)
        self.mlp = MLP(2*in_channels,fc_dims)
        self.mlp_edge = MLP(2*in_channels+nedge_features,fc_dims)
        self.edge_model = edge_model
        

    def forward(self, x, edge_index, edge_attr):
        """
        Includes Edge Features along with Node Features for updating nodes. 
        Args:
        ...
        Returns: Updated Node feature matrices
        """
        # x has shape [N, in_channels]
        # edge_index has shape [2, E]
        # edge_attr has shape [E, Edge_channels] Ex4
        if self.edge_model is None:
            return self.propagate(edge_index, size=(x.size(0), x.size(0)), x=x, edge_f=None)
        
        else:
            return self.propagate(edge_index, size=(x.size(0), x.size(0)), x=x, edge_f=edge_attr)


    def message(self, x_i, x_j,edge_f):
        # x_i has shape [E, in_channels]
        # x_j has shape [E, in_channels]

        temp = torch.cat((x_i,x_j-x_i,edge_f),dim=1)   # tmp has shape [E, 2 * in_channels+edge_Ch]
        if edge_f is None:
...
class Edge_feat(MessagePassing):
    """
    Edge Feature Multiplication Message Passing Network Class. Extracted from torch_geometric, with minor modifications.
    (https://rusty1s.github.io/pytorch_geometric/build/html/modules/nn.html)
    """
    def __init__(self, in_channels,fc_dims, out_channels, edge_model=None, nedge_features=0):
        super(Edge_feat, self).__init__(aggr='max') #  "Max" aggregation.
        fc_dims.append(out_channels)
        self.mlp = MLP(2*in_channels,fc_dims)
        self.mlp_edge = MLP(2*in_channels+nedge_features,fc_dims)
        self.edge_model = edge_model
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        if self.edge_model is None:
            ...
            temp = torch.cat((x_i,x_j-x_i,edge_attr),dim=1)          # E,(2*in_channels+edge_Ch)
    ...
    def message(self, x_i, x_j):
        ...
        temp = torch.cat((x_i,x_j-x_i),dim=1)   # tmp has shape [E, 2 * in_channels+edge_Ch]
...
class GGC_Net(torch.nn.Module):     
    """
    Gated Graph Conv Network with Node and Edge Features Class used to peform the node and edge update during Neural message passing.
    """       
    def __init__(self, edge_in_dims = None, edge_out_dims = None, edge_fc_dims = None, node_in_dims = None, node_out_dims = None, 
                 node_fc_dims = None, ggc_layers = None, ggc_out_dims = None, use_batchnorm = None, dropout = 0.1):
        super(GGC_Net, self).__init__()
        
        if edge_in_dims is not None:
            self.edge_mlp = Edge_feat(node_in_dims,edge_fc_dims,edge_out_dims,1,nedge_features=edge_in_dims)
            self.lin_edge = torch.nn.Linear(edge_out_dims,edge_in_dims)
        else:
            self.edge_mlp = None        
       
        if node_in_dims is not None:
            self.node_mlp = Node_feat(node_in_dims,node_fc_dims,node_out_dims,1,nedge_features=edge_in_dims)
            self.lin_node = torch.nn.Linear(ggc_out_dims,node_in_dims)
        else:
            self.node_mlp = None

        self.ggconv1 = GatedGraphConv(ggc_out_dims,ggc_layers)
        self.act2 = torch.nn.ReLU()
        
        

    def forward(self, data, dual=1):
        x, edge_index, edge_attr, batch = data.x, data.edge_index, data.edge_attr, data.batch
        #print(edge_attr.shape)
        edge_attr_up = edge_attr

        if dual == 0:   
            if self.node_mlp is not None:
                x = self.node_mlp(x,edge_index,edge_attr)
                x = self.ggconv1(x,edge_index)
                x = self.lin_node(x)
                x = self.act2(x)
            else:
                print("Error : node_input dimensions not specified.")
                return None;
            
        else:
            
            edge_attr_up = self.edge_mlp(x,edge_index,edge_attr)
            edge_attr_up = self.lin_edge(edge_attr)
        ...
        return x,edge_attr_up

class mod_call(torch.nn.Module):
    def __init__(self):
        super(mod_call, self).__init__()
        self.model1 = GGC_Net(edge_in_dims = 4, edge_out_dims = 4, edge_fc_dims = [5,10], node_in_dims = 1, node_out_dims = 1, 
                              node_fc_dims = [2,10], ggc_layers = 3, ggc_out_dims = 10, use_batchnorm = None, dropout = 0.1)
        
        self.model2 = Gat_Net(edge_in_dims = 4, edge_out_dims = 4, edge_fc_dims = [5,10], node_in_dims = 2, node_out_dims = 2, 
                node_fc_dims = [2,10], gat_in_dims = 2, gat_out_dims = 2, gat_fc_dims = [4], gat_head = [4,1], use_batchnorm = None, dropout = 0.1)
    

    def forward(self,data):

        return self.model2(data,dual = 1)

class Gat_Net(torch.nn.Module):  
    """
    Graph Attention Network with Edge Features Class used to peform the node update during Neural message passing.
    """          
    def __init__(self, edge_in_dims = None, edge_out_dims = None, edge_fc_dims = None, node_in_dims = None, node_out_dims = None, 
        node_fc_dims = None, gat_in_dims = None, gat_out_dims = None, gat_fc_dims = None, gat_head = None, use_batchnorm = None, dropout = 0.1):
        
        super(Gat_Net, self).__init__()
        
        gat_fc_dims.append(gat_out_dims)
        if edge_in_dims is not None:
            self.edge_mlp = Edge_feat(node_in_dims,edge_fc_dims,edge_out_dims,1,nedge_features=edge_in_dims)
            self.lin_edge = torch.nn.Linear(edge_out_dims,edge_in_dims)
        else:
        ...
        if node_in_dims is not None:
            self.node_mlp = Node_feat(node_in_dims,node_fc_dims,node_out_dims,1,nedge_features=edge_in_dims)
            self.lin_node = torch.nn.Linear(gat_out_dims,node_in_dims)
        else:
            self.node_mlp = None

        #self.conv1 = GATConv(gat_in_dims, gat_fc_dims,heads=gat_head)
        #self.conv2 = GATConv(gat_head*gat_fc_dims, gat_out_dims,heads=1,concat=False)
        self.conv1 = GAT_Model(gat_in_dims, gat_fc_dims,heads=gat_head,dropout_p=dropout)
        self.act2 = torch.nn.ReLU()     
...
class GCN_Net(torch.nn.Module):  
    """
    Graph Convolution Network with Edge Features Class used to peform the node update during Neural message passing.
    """          
    def __init__(self):
        super(Gat_Net, self).__init__()
        
        self.ed = Edge_feat(1,[10],[8])
        self.conv1 = GCNConv(8,64,bias=True)
        self.conv2 = GCNConv(64,256,bias=True)
        self.conv3 = GCNConv(256,32,bias=True)
        self.act1 = torch.nn.Tanh()
        self.act2 = torch.nn.ReLU()
        self.lin1 = torch.nn.Linear(32,1)

    def forward(self, data):
        x, edge_index, edge_attr, batch = data.x, data.edge_index, data.edge_attr, data.batch
        
        x = self.ed(x,edge_index,edge_attr)
        x = F.dropout(x, p=0.6, training=self.training)
...
class Net(torch.nn.Module):  
    ...
    def __init__(self):
        ...
        self.ed = Edge_feat(1,10,20,10)
        self.conv1 = GATConv(10, 20,heads=8)
        self.conv2 = GATConv(8*20, 10,heads=1,concat=False)
        self.ggconv1 = GatedGraphConv(10,3,bias=True)
        
        self.act2 = torch.nn.ReLU()
        self.lin1 = torch.nn.Linear(10,1)
    ...
    def forward(self, data):
        ...
        #x = self.conv1(x,edge_index)
        #x = self.act2(x)
        #x = self.ed2(x,edge_index,edge_attr)
        #x = self.conv2(x,edge_index)
        #x = self.act2(x)
...
class MetaLayer(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        if self.edge_model is not None:
            edge_attr = self.edge_model(edge_index, edge_attr)
    ...
    def __init__(self, model_params):
        ...
        Args:
            model_params: dictionary contaning all model hyperparameters
                         1. encoder_feats_dict - node_out_dim, edge_out_dim
                         2. classifier_feats_dict - edge_details**
                         3. num_enc_steps
                         4. num_class_steps
                         5. node_agg_fn - max,mean,sum
                         6. reattach_initial_nodes
                         7. reattach_initial_edges
                         8. edge_model_feats_dict - fc_dims, dropout_p, use_batchnorm
                         9. node_model_feats_dict - fc_dims, dropout_p, use_batchnorm
        """
